pub mod review_signing {
    ...
    use crate::{
        api::models::app_reviews::AppReviewSignatureData, config::Config, constants,
        errors::ServiceError,
    };
    ...
    pub fn create_or_load_review_signing_key(config: &Config) -> Result<SigningKey, String> {
        ...
        if !private_key_path.exists() || !public_key_path.exists() {
            ...
            {
                Ok(public_key_pem) => match std::fs::write(&public_key_path, public_key_pem) {
            ...
            match signing_key.to_pkcs8_pem(LineEnding::LF) {
                Ok(private_key_pem) => match std::fs::write(&private_key_path, private_key_pem) {
    ...
    pub fn sign_review(
        review_data: &AppReviewSignatureData,
        ...
        let review_data_object = match serde_json::to_value(review_data) {
            ...
            Err(e) => {
                ...
                return Err(ServiceError::InternalServerError {
                    error_message: "Failed to serialize review data".to_string(),
                });
            }
        };
