...
impl OAuth2State {
    pub fn preconfigured(config: Config) -> Self {
        ...
        let oauth_config = match fs::read_to_string(&config.oauth_config_path) {
            Ok(oauth_config_toml_string) => {
                toml::from_str::<OAuthConfig>(&oauth_config_toml_string)
                    .map_err(|e| log::error!("Failed to read oauth configuration: {:?}", e))
                    .ok()
            }
            Err(e) => {
                log::warn!(
                    "Couldn't load oauth config from path {}: {:?}",
    ...
    fn handle(
        ...
        match ex {
                ...
                let solicitor =
                    FnSolicitor(move |_: &mut OAuthRequest, solicitation: Solicitation| {
                        ...
                        let redirect_url = url::Url::parse_with_params(
                            &format!("{}/auth/authorize", &public_url),
                            &extra,
                        )
                        .unwrap();
                        ...
                        response
                            ...
                            .map_err(|_| {
                                WebError::InternalError(Some("Failed to redirect".to_string()))
                            })
                            .unwrap();
            ...
            Extras::AuthPost(user_id, result) => {
                let solicitor =
                    FnSolicitor(move |_: &mut OAuthRequest, _: Solicitation| match result {
                        OAuth2AuthorizationResult::Allow => {
                            OwnerConsent::Authorized(user_id.clone())
                        }
            ...
            //     op.run(self.with_solicitor(solicitor))
            // },
            Extras::Nothing => op.run(&mut self.endpoint),
